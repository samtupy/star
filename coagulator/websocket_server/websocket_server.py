...
class WebSocketHandler(StreamRequestHandler):
    ...
    def read_next_message(self):
        ...
        opcode_handler(self, message_bytes.decode('utf8') if opcode == OPCODE_TEXT else bytes(message_bytes))
    ...
    def send_text(self, message, opcode=OPCODE_TEXT):
        ...
        if isinstance(message, bytes):
            message = try_decode_UTF8(message) if opcode == OPCODE_TEXT else message # this is slower but ensures we have UTF-8
            if not message:
        ...
        header  = bytearray()
        payload = encode_to_UTF8(message) if opcode == OPCODE_TEXT else message
        payload_length = len(payload)
